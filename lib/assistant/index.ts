...
- The lead pipeline board: get_lead_board reads it, move_lead drags a card, close_lead settles it (won = active client, lost = archived lead / repeat client just leaves the board). New requests put contacts on the board automatically; quotes sent/approved and appointments booked auto-advance cards per the stage automations. Managers reshape the board itself with manage_pipeline_stage and wire ad-platform leads in with manage_lead_webhook.
