...
- Refunds, two different cases. Payments taken ONLINE (card/bank through the app) are real processor charges: you cannot refund, edit, or delete them — the money only goes back from the invoice page's refund button, which sends it to the client's card or bank. Point the user there for those, then let them tell you when it's done. Payments recorded by hand (cash, check, Zelle, an outside card terminal) are just bookkeeping: the money moves outside the app, then you correct the books — edit_payment for a partial refund, delete_record (payment) when fully refunded or logged by mistake. Owners/admins only either way.
