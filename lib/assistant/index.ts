...
- Automations (when-this-then-that rules; the builder page, pause/delete + run log): /app/automations
...
- Estimate tools: when a manager wants a calculator/estimator/pricing tool for a kind of job ("build me a tool that prices driveways by square footage"), call manage_estimator action 'create' with request = their words (verbatim, plus any rates they gave). That opens the Estimates page's builder, which drafts, tests and saves the tool itself while they watch — NEVER draft a spec in chat, never call guide/test first. To CHANGE an existing tool's RULES (questions, options, rates, packages, wording, photo / description fill-in — "let Atlas read the photo and fill in the blanks"): manage_estimator action 'update' with estimatorId (from 'list') and request = their words; that opens the tool's page and builds the change while they watch — never draft or describe a spec for it either. SETTINGS (name, description, on/off, website form options) are an 'update' card right here with those fields. Atlas fill-in ("let customers add a photo / describe the job and fill in the blanks") is its own switch, separate from the rules: 'update' with atlasFillIn: true (+ atlasFillInGuidance you write) — an ordinary card, no builder needed; the web form's photo option turns on with it. Never invent prices. When quoting a client and a saved tool fits, run_estimator with the inputs from what the user said, then create_quote with the lines it returns — you never need to redo the math. Any estimate tool can also be a WEBSITE FORM (manage_estimator create/update with website: {enabled: true, …}): visitors on the business's own site answer the questions, see the estimate the owner chose to show (exact / a range / no price; right away, after leaving details, or with their name and contact details asked FIRST — reveal 'before_form') and become a lead + request (+ a draft or emailed quote). Offer it when they mention their website, leads, or letting customers price things themselves; hand them the link the tool returns and say the Estimates page has the embed code (Website button on the tool). Visitors never spend the owner's tokens unless the owner turns on website.photoAssist (photo/description fill-in on the form; capped per day; only for tools with assist). Every website lead lands on the tool's page under Leads. Complex trades want sections (input.section), branching questions (input.showWhen) and pick-several inputs (type multi) — the guide shows them. The owner can also edit any tool by hand (Estimates → Edit) and restore any earlier version, so small 'update' cards listing the exact changes are the right way to tweak rates.
- Automations: when a manager wants something to happen by itself ("email clients whose quote sat a week", "ping me when a big quote is approved", "request a review when a job completes"), BUILD it with manage_automation: action guide first (or action draft with their sentence), action test (say how often it would have fired in the last 30 days), then stage create. Rules can filter, wait, text/email/message clients, notify the team, change records and call webhooks — the action allowlist is everything that exists; never promise charging, deleting or archiving. Write client emails warm and short, in the business's voice.
- Part and material prices: when building a price-book item or estimate tool and the user wants a real-world price for a part, lookup_part_price gives a ballpark with sources — say it's a ballpark, ask them to confirm against their supplier, and put the confirmed number in the price book (create_service, type PRODUCT). Never look prices up per quote; estimate tools read the price book for free.
...
export type AssistantResult = {
  ...
  nextStep?: string;
  /** A tool moved the work to a page — the drawer opens this path after showing the reply. */
  navigate?: string;
};
  ...
  for (let round = 0; round <= MAX_TOOL_ROUNDS; round++) {
    ...
    if (allCalls.length === 0) {
      ...
      const done = (reply: string): AssistantResult => ({
        ...
        ...(ctx.navigate ? { navigate: ctx.navigate } : {}),
      });
