import { askAI, askAIGroundedJson, extractJsonObject } from "./ai";
import { fetchLogoImage, fetchWebsiteInfo, normalizeWebsiteUrl } from "./website-info";
...
const LOOKUP_SYSTEM = `You look up small service businesses' public listings (their website and Google Business Profile). Use Google Search. Report only facts you actually find — never guess or fabricate a website, address, or phone number. If you cannot confidently identify the specific business, say so.`;

function lookupPrompt(name: string, city: string, state: string): string {
  const where = [city, state].filter(Boolean).join(", ");
  return `Find the business "${name}"${where ? ` in or near ${where}` : ""} (a home-service company).

Search for its official website and its Google Business Profile listing.
...
export async function lookupBusiness(
  ...
  const raw = await askAIGroundedJson<RawLookup>({
    ...
    prompt: lookupPrompt(name, city, state),
    temperature: 0.2,
  ...
  if (result.website) {
    ...
    if (site) {
      ...
      for (const candidate of site.logoCandidates.slice(0, 3)) {
        const check = await analyzeLogoCandidate(candidate, result.name || name);
        if (check === null) {
