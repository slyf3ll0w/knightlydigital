...
- 2026-09-16 Batch E (dead-ends + guards) — #13 convertQuoteToJob claims the quote (updateMany on jobId null) before creating the job, convert route 400s QuoteAlreadyConvertedError, public approval claims status, Convert buttons disabled while busy, scripts/ensure-deposit-invoice-index.mjs (partial unique index, in db:backfill); #17 invoice delete → job REQUIRES_INVOICING; #18 archived stays archived on edit; #19 job-linked invoices scoped by jobScope (prefill + POST); #25 lib/quote-expiry.ts end-of-day in company tz; #27 lib/quote-approval.ts shared by public approve + internal Mark Approved (limited to AWAITING/CHANGES_REQUESTED); #28 emailed:false surfaced (collect-deposit, booking accept/decline, approval) + activity log email_failed; #32 POST contacts/[id]/portal-reset + Reset portal link (managers); #38 deactivate unassigns future JobAssignments + strips visitAssigneeIds, TeamClient reports it; #39 booking charge throw → look up transfer by tags.invoiceId (findUnrecordedTransferForInvoice) else unwind; payment record retried then Sentry; autopay idempotency keyed by attempt + same lookup on transient; #40 geocode country/proximity bias + acceptGeocodeMatch (low confidence / other state rejected); #44 withSerializationRetry ×3 on both booking txs; #45 per-IP limit on public booking POSTs, per-token+IP on hub requests; #46 TECH excluded from appointment assignee (POST/PATCH/pickers); #47 status dropped from job PATCH; #48 quote create checks request.contactId; #49 sanitizeDeposit on create; #50 first payment on a DRAFT stamps issuedAt/dueDate. Tests: scripts/test-ops-guards.ts (4). Status: ALL FIVE BATCHES DONE on fix/ops-batch-ae (A+E by the parent session, B/C/D merged in).
