...
import { PhoneIncoming, PhoneOutgoing, PhoneMissed, Voicemail, PhoneOff, Headphones } from "lucide-react";
...
export default function CallRow({
  ...
  const showCallBack = canCall && call.status !== "RINGING" && call.status !== "IN_PROGRESS" && Boolean(call.customerNumber && call.customerNumber !== "unknown");

  return (
    <div className={`card-ledger px-4 py-3 ${unseen ? "border-l-2 border-l-red-400" : ""}`}>
      <div className="flex items-start gap-3">
        <Tile call={call} />
        <div className="min-w-0 flex-1">
          <div className="flex items-baseline gap-2">
            {showContact && call.contact ? (
              <Link href={`/app/contacts/${call.contact.id}`} className={`truncate text-[15px] hover:underline ${unseen ? "font-bold text-gray-900" : "font-semibold text-gray-800"}`}>
                {label}
              </Link>
            ) : (
              <p className={`truncate text-[15px] ${unseen ? "font-bold text-gray-900" : "font-semibold text-gray-800"}`}>{showContact ? label : call.direction === "INBOUND" ? "Incoming call" : "Outgoing call"}</p>
            )}
