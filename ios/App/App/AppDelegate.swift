...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    // The AppShortcuts plugin only sees quick-action taps that the app
    // forwards to it (retained until the web listener attaches, so the
    // cold-start race is safe).
    func application(_ application: UIApplication, performActionFor shortcutItem: UIApplicationShortcutItem, completionHandler: @escaping (Bool) -> Void) {
        NotificationCenter.default.post(name: NSNotification.Name(AppShortcutsPlugin.notificationName), object: nil, userInfo: [AppShortcutsPlugin.userInfoShortcutItemKey: shortcutItem])
        completionHandler(true)
    }

    // The Capacitor PushNotifications plugin listens for these NotificationCenter
    // posts. iOS hands us an APNs token; the server sends via FCM, so exchange
    // it and surface the FCM token to the plugin's `registration` event.
    func application(_ application: UIApplication, didRegisterForRemoteNotificationsWithDeviceToken deviceToken: Data) {
        ...
        Messaging.messaging().token { token, error in
            if let token = token {
                ...
                NotificationCenter.default.post(name: .capacitorDidFailToRegisterForRemoteNotifications, object: error ?? NSError(domain: "FCM", code: -1))
            }
        }
    }
    ...
    func applicationDidEnterBackground(_ application: UIApplication) {
        // No call up: drop the Telnyx registration cleanly, so a socket iOS is
        // about to freeze never lingers at Telnyx as a registration a call could
        // be routed to. The next VoIP push registers afresh.
        VoipEngine.shared.appBackgrounded()
    }

    func applicationWillEnterForeground(_ application: UIApplication) {
        VoipEngine.shared.appForegrounded()
    }
