...
export async function generateStaticParams() {
  ...
  for (const citySlug of getAllCitySlugs()) {
    for (const svc of services) {
      params.push({ city: citySlug, service: svc.slug });
    }
  }
  return params;
}
      ...
      <section className="relative min-h-[55vh] flex flex-col justify-end overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-5 pb-16 w-full">
          <nav className="flex items-center gap-2 text-xs text-white/50 mb-6 font-medium tracking-wider">
            ...
            <span>/</span>
            <Link href={`/${citySlug}`} className="hover:text-white/80 transition-colors">{city.name}</Link>
            <span>/</span>
            <span className="text-white/80">{svc.shortName}</span>
          </nav>
          <p className="text-xs tracking-[0.3em] font-bold uppercase text-accent/80 mb-3">
            {city.name}, TX
          </p>
          <h1 className="text-4xl lg:text-6xl font-black uppercase text-white leading-tight max-w-3xl">
            {svc.name}
          </h1>
        </div>
      </section>
      ...
      <section className="bg-patterned py-20">
        <div className="max-w-7xl mx-auto px-5 grid lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2">
              ...
              {svc.details.map((d, i) => (
                <div
                  ...
                  className={`flex items-start gap-5 px-6 py-5 ${
                    i < svc.details.length - 1 ? "border-b border-border" : ""
                  }`}
                >
            ...
            {/* Other services */}
            <div className="mt-14">
              <p className="text-xs tracking-[0.25em] font-bold uppercase text-muted-foreground mb-5">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                {otherServices.map((s) => (
                  <Link
                    ...
                    className="border border-border p-6 hover:border-primary hover:bg-muted transition-colors group"
                  >
          ...
          <div>
            <div className="bg-primary text-primary-foreground p-8 mb-6">
              ...
              <p className="text-sm text-primary-foreground/70 mb-6">
                Talk to us about {svc.shortName} for your {city.name} business.
              </p>
