...
type Panel = { id: string; mode: "approve" | "resolve" | "decline" } | null;
...
export default function FeedbackClient({ tickets }: { tickets: Ticket[] }) {
  ...
  function openPanel(t: Ticket, mode: NonNullable<Panel>["mode"]) {
    setError("");
    setPanel({ id: t.id, mode });
    if (mode === "approve") {
  ...
  function Card({ t }: { t: Ticket }) {
    const active = panel?.id === t.id ? panel : null;
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-5">
        ...
        {!active && (
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            {t.status !== "OPEN" && (
              <button
                onClick={() => patch(t.id, { action: "reopen" })}
                disabled={busy === t.id}
              ...
              >
                {busy === t.id ? <Loader2 size={12} className="animate-spin" /> : <RotateCcw size={12} />}
                Reopen
              </button>
            )}
            <button
          ...
          <div className="mt-4 space-y-2.5 rounded-lg border border-blue-200 bg-blue-50/50 p-4">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                disabled={busy === t.id || !draftTitle.trim()}
                className="inline-flex items-center gap-1.5 rounded-lg bg-[#0B57D8] px-4 py-2 text-sm font-semibold text-white hover:bg-[#0A4CBB] disabled:opacity-50"
              >
                {busy === t.id ? <Loader2 size={14} className="animate-spin" /> : <Megaphone size={14} />}
                Post to board
              </button>
              <button
        ...
        {/* ── Resolve / decline — optional reply ── */}
        {(active?.mode === "resolve" || active?.mode === "decline") && (
          <div className="mt-4 space-y-2.5 rounded-lg border border-gray-200 bg-gray-50 p-4">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => patch(t.id, { action: active.mode, response: draftResponse })}
                disabled={busy === t.id}
