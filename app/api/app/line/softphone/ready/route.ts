...
import { siblingMembershipFor } from "@/lib/voip";
 ...
 * start switching before its softphone has even registered.
 */
export async function POST(req: NextRequest) {
  const actor = await getActor();
  if (!actor) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  const body = (await req.json().catch(() => ({}))) as { callId?: unknown; probe?: unknown };
