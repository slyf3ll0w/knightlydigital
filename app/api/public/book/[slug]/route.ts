...
export async function POST(req: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  ...
  const result = await withDocNumberRetry(() =>
    prisma.$transaction(async (tx) => {
      ...
      if (contact && smsConsent && (!contact.smsConsentAt || contact.smsDisabled)) {
        // Fresh opt-in from a known client — turns texts back on if they were
        // off and stamps consent once. A blank box never turns a known client off.
        contact = await tx.contact.update({
          where: { id: contact.id },
          data: {
            smsDisabled: false,
            ...(!contact.smsConsentAt ? { smsConsentAt: new Date(), smsConsentSource: hubContact ? "hub" : "booking" } : {}),
          },
        });
      }
      if (!contact) {
        contact = await tx.contact.create({
          data: {
            ...
            ...(smsConsent
              ? { smsConsentAt: new Date(), smsConsentSource: hubContact ? "hub" : "booking" }
              : smsDeclined
